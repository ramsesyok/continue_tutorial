...
- Continue を使う際のデータフローを理解し、エアギャップ環境での安全性の根拠を把握する
- 機密コードを誤って LLM に送信しないための設定・ポリシーを整備する
- Continue が出力するログの種類・場所・管理方法を習得する
- VS Code 拡張と config.yaml の配布・更新を組織的に運用する手順を学ぶ
...
Continue は、ユーザーが入力したプロンプトや選択したコードを LLM に送信することで動作します。ここでは「何がどこに送られるのか」を整理し、エアギャップ環境でのセキュリティ上の根拠を確認します。
...
ローカル LLM（Ollama・vLLM・LM Studio など）を使用している場合、コードとプロンプトはすべてイントラネット内で完結します。インターネット上の外部 API には一切送信されません。
...
- **プロンプトテキスト**: ユーザーがチャットや Edit で入力したテキスト
- **コードコンテキスト**: 選択したコード、`@file` で指定したファイル、`@codebase` による検索結果など
- **システムプロンプト**: Rules（`~/.continue/rules/` 配下のファイル）の内容

いずれもローカル LLM エンドポイントにのみ送信されます。ただし、**contextLength の設定値まで自動的にコードが含まれる**点に注意が必要です。次節でこの点を詳しく説明します。
...
Continue は、ユーザーが明示的に選択したコード以外にも、周辺コードを自動的にコンテキストとして LLM に送ります。この範囲は `config.yaml` の `contextLength` で制御されます。
...
!!! warning "Autocomplete のコンテキストについて"
    Autocomplete（タブ補完）では、カーソル前後のコードが自動的にコンテキストとして使われます。機密性の高いファイルを編集する際は、Autocomplete を一時的に無効化することも検討してください（`Ctrl+Shift+P` → `Continue: Toggle Autocomplete`）。

### .continueignore でファイルを除外する

`.continueignore` ファイルを使うと、特定のファイルやディレクトリを Continue の補完・インデックス・コンテキスト検索から除外できます。書式は `.gitignore` と同一です。
...
特定バージョンを固定して全社配布するには、VSIX ファイルを社内 Web サーバや共有ドライブに置き、バージョンごとにディレクトリを分けて管理する方法が簡単です。
...
| ワークスペース設定 | `<リポジトリルート>/.continue/config.yaml` | プロジェクト共通の Rules・ツール設定など |
...
LLM は誤ったコードや、古い API を使ったコードを返すことがあります（ハルシネーション）。
...
エアギャップ環境では起きにくいですが、プロキシ経由で Marketplace にアクセスできる場合は注意が必要です。
