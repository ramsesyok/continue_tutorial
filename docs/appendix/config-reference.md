...
| `embeddingsProvider` | オブジェクト | — | ローカル Embedding モデルの設定 | [embeddingsProvider](#embeddingsprovider) / 第 10 章 |
...
| `api_base` | 文字列 | — | プロバイダ依存 | LLM サーバのエンドポイント URL。プロバイダのデフォルトから変更する場合に指定します |
...
Ollama のデフォルトポートは `11434`、`api_base` を省略すると `http://localhost:11434` が使われます。
...
| 固有フィールド | 型 | 説明 |
| --- | --- | --- |
| `api_base` | 文字列 | Ollama サーバの URL。別ホストで動かす場合に指定します |

#### openai（vLLM / LM Studio OpenAI 互換モード含む）

vLLM や LM Studio の OpenAI 互換 API にも `provider: openai` を使います。`api_base` でエンドポイントを社内サーバに向けてください。
...
| 固有フィールド | 型 | 説明 |
| --- | --- | --- |
| `api_base` | 文字列 | OpenAI 互換サーバの URL。末尾 `/v1` まで含めて指定します |
...
LM Studio 専用プロバイダです。デフォルトポート `1234` が自動で適用されるため、同一マシンで動かす場合は `api_base` を省略できます。
...
| `model` | 文字列 | — | モデル名（必須） |
| `api_base` | 文字列 | プロバイダ依存 | エンドポイント URL |
...
`@codebase` や `@docs` 機能でコードベースをベクトル検索するために使うローカル Embedding モデルを設定します（→ 詳細は [第 10 章](../10-internal-models-context.md)）。

### フィールド一覧

| フィールド | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `provider` | 文字列 | ✓ | `ollama` または `openai`（互換サーバ向け） |
| `model` | 文字列 | ✓ | Embedding モデルの名前 |
| `api_base` | 文字列 | — | Embedding サーバのエンドポイント URL |
| `api_key` | 文字列 | — | 認証トークン（不要な場合は省略可） |
...
| `transport` | オブジェクト | ✓ | 接続方式の設定。`type` に `stdio` または `sse` を指定します |
...
| --- | --- | --- | --- |
| `type` | 文字列 | ✓ | `"sse"` を指定 |
| `url` | 文字列 | ✓ | MCP サーバの SSE エンドポイント URL |
