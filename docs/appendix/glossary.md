...
ソフトウェアが動作状況・利用傾向などのデータをネットワーク越しに自動送信する仕組みのことです。Continue はデフォルトで匿名の使用統計を送信する設定になっています。エアギャップ環境では VS Code の `settings.json` に `"continue.telemetryEnabled": false` を設定して無効化し、パケットキャプチャなどで実際に送信が止まっていることを確認します。初出 → [第 4 章](../04-telemetry-airgap-verification.md)
