...
- **再クロール方法**: VS Code のコマンドパレット（`Ctrl+Shift+P` / `Cmd+Shift+P`）から `Continue: Reindex Docs` を実行するか、対象 `docs` エントリの設定を一度削除して保存→再追加することで再構築されます
...
| モデル | パラメータ規模 | 主な特徴 |
| --- | --- | --- |
| `nomic-embed-text` | 約 137M | 軽量・高速。汎用テキストに強い。Ollama で標準的に動作 |
...
!!! note "モデルの入手はオフラインで"
    上記モデルは公開モデルですが、エアギャップ環境では事前にミラー／オフライン配布した上で社内 Ollama や vLLM 上にロードしておく必要があります。インターネット経由でのモデルダウンロードは行いません。
...
- **`@codebase` の万能視**: 「ドキュメントに書いてあること」をコードから推定させようとすると、ハルシネーションが増えます。文書ベースの質問は `@docs` を使う方が安全です（第 6 章でも触れたとおり、ハルシネーション抑止にはコンテキストの質が重要です）
...
- **ログの設計**: どの利用者がいつ何を取得したかをサーバ側で記録し、監査ログとして残せるようにする（第 13 章で扱う運用観点と連動）
