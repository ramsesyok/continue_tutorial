...
- Continue がデフォルトで送信しうるテレメトリデータの種類を理解する
- `config.yaml` と VS Code の設定でテレメトリを無効化する方法を習得する
- VS Code 開発者ツール・プロキシログ・パケットキャプチャの 3 通りの手順で外部通信の有無を検証できるようになる
...
テレメトリ（telemetry）とは、ソフトウェアが動作状況や利用傾向などのデータを、ネットワーク越しに自動送信する仕組みのことです。VS Code 本体や各拡張機能は、開発元が製品改善を目的としてこの仕組みを組み込んでいる場合があります。

オンプレミス・エアギャップ環境では、テレメトリによる外部通信を許可しないことが一般的なセキュリティポリシーとなっています。本章では、Continue におけるテレメトリ設定の無効化と、その効果を検証するための具体的な手順を説明します。

### Continue が収集・送信しうるデータ

Continue はデフォルト設定のままだと、以下のような匿名の使用統計データを送信することがあります。

- 機能の呼び出し回数（Chat、Autocomplete、Edit などの利用頻度）
...
### `allowAnonymousTelemetry` の設定

Continue のテレメトリは `config.yaml` に以下の 1 行を追加することで無効化できます。

```yaml
# ~/.continue/config.yaml（Linuxの場合）
# %USERPROFILE%\.continue\config.yaml（Windowsの場合）
...
!!! tip "設定ファイルの場所"
    `config.yaml` は OS によってパスが異なります。VS Code のコマンドパレット（`Ctrl+Shift+P`）から「Continue: Open config.yaml」と入力すると、直接ファイルを開けます。

設定を変更したら VS Code を再起動し、変更を反映させてください。

### VS Code 設定との関係

Continue のテレメトリ制御には、`config.yaml` の `allowAnonymousTelemetry` に加えて、VS Code の `settings.json` も関係します。
...
| 設定箇所 | キー | 影響範囲 |
| --- | --- | --- |
| `config.yaml` | `allowAnonymousTelemetry` | Continue 拡張全体 |
| VS Code `settings.json` | `continue.telemetryEnabled` | ワークスペースまたはユーザー単位 |

どちらか一方でも `false` に設定されていれば、テレメトリは無効化されます。確実を期すために**両方**を設定することを推奨します。
...
6. ネットワークタブにリクエストが表示されないこと、または表示されるリクエストがすべてローカル LLM ホスト（`<your-internal-llm-host>`）宛であることを確認する
...
■ 設定変更
  [ ] config.yaml に allowAnonymousTelemetry: false を記載した
...
- `config.yaml` の `allowAnonymousTelemetry: false` と、VS Code の `continue.telemetryEnabled: false` を両方設定することで無効化できる
- 無効化後は、開発者ツール・プロキシログ・パケットキャプチャのいずれかの手順で外部通信がないことを検証する
- 検証結果はスクリーンショットやログとして保存し、エビデンスチェックリストとあわせて監査担当者に提出する

## 次の章へ

次は [第5章 config.yaml の基本構造](05-config-yaml-basics.md) で、Continue 全体の設定ファイルの書き方を詳しく学びます。本章で触れた `allowAnonymousTelemetry` をはじめ、モデルの接続先や各機能のパラメータをどのように記述するかを理解できます。
