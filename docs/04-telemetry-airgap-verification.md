...
!!! tip "settings.json の場所"
    VS Code のコマンドパレット（`Ctrl+Shift+P`）から「Preferences: Open User Settings (JSON)」と入力すると、ユーザー設定の `settings.json` を直接開けます。ワークスペース単位で設定する場合は「Preferences: Open Workspace Settings (JSON)」を使用してください。
