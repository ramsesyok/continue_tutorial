...
- ローカル <glossary:LLM> サーバが正常に稼働していることを `curl` コマンドで独立して確認できる
...
| Continue 拡張 | 0.9 以上 | <glossary:VSIX> 形式で社内配布されていること |
| curl | 7.68 以上 | LLM サーバへの疎通確認に使用 |
| ローカル LLM サーバ | 各製品の最新安定版 | <glossary:Ollama> / <glossary:vLLM> / <glossary:LMStudio> のいずれか |
...
- **vLLM**: <glossary:OpenAI互換API> をポート `8000` で公開する高スループット<glossary:推論>エンジン
...
1. `config.yaml` に LLM <glossary:エンドポイント>の情報が正しく記述されている
2. Continue の <glossary:Chat> パネルからモデルの応答が得られる
...
!!! note "参考リンクについて"
    上記リンクはすべて外部サイトです。<glossary:エアギャップ>環境からは接続できません。
    事前にオフラインで取得したドキュメントや、<glossary:社内ミラー>を参照してください。
